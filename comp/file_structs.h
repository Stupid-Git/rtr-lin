...
#ifndef COMP_FILE_STRUCTS_H_
#define COMP_FILE_STRUCTS_H_
...
typedef struct fact_config_S {             // 本体設定（３２ビット境界であること）
...
} fact_config_t __attribute__((aligned(4)));


// size 8192 byte
typedef struct sys_config_S {             // 本体設定（３２ビット境界であること）
...
// sakaguchi 2020.11.11 ↑
    ...
    uint32_t crc;                   // 4 byte
    // 8192
} my_config_t __attribute__((aligned(4)));

//typedef uint8_t group_data_S[SFM_GROUP_SIZE] group_data_t;
extern uint8_t group_data[SFM_GROUP_SIZE];
extern uint8_t regist_data[SFM_REGIST_SIZE_32];
extern uint8_t temp_data[SFM_TEMP_SIZE];




#endif /* COMP_FILE_STRUCTS_H_ */
