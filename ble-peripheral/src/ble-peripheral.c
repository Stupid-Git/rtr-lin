...
// https://github.com/leandroosalas/ble-peripheral
// sudo apt-get install libbluetooth-dev

// library # gcc -o simplescan simplescan.c -lbluetooth

// https://www.howtogeek.com/829360/how-to-set-up-bluetooth-on-linux/

// Old bluetooth 4.0 dongle seems to work (Kubuntu 20.04) 5.0 dongle was NG

/*
 https://stackoverflow.com/questions/30386577/c-c-ble-read-write-example-with-bluez

Bluez (Linux official Bluetooth stack) has moved to DBUS for its API. While in the past,
it was 'accepted' to 'fork' bluez code to access BLE support on Linux, now the approach
is to use DBUS. Bluez DBUS API is documented here: https://git.kernel.org/pub/scm/bluetooth/bluez.git/tree/doc

Either you directly speak to DBUS for your bluetooth C/C++ application or you use a GATT library as a helper.
The second one is probably the best approach for beginner (and also for non-beginner who would prefer to keep
their program Bluez agnostic for instance to support other OSes or Bluez pre-DBUS API or to have their sources
more readable).

One of these GATT libraries that support modern D-BUS API is gattlib (note: I am the author of this library).
Here is a simple example based on this library for reading/writing a BLE device:
 ...
 https://github.com/labapart/gattlib/blob/master/examples/nordic_uart/nordic_uart.c
*/
...
//scan data
//static uint8_t scanRspData[] = { 0x09, 0x08, 0x55, 0x32, 0x46, 0x42, 0x6c, 0x75, 0x65, 0x7a }; // U2FBLUEZ
  static uint8_t scanRspData[] = { 0x09, 0x08, 0x55, 0x33, 0x46, 0x42, 0x6c, 0x75, 0x65, 0x7a }; // U3FBLUEZ
...
printf("start advertising selectRetval = %d\n", selectRetval);
...
printf("2 hci_le_set_advertise_enable = %d\n", selectRetval);
    		// start advertising
    		hci_le_set_advertise_enable(hciSocket, 1, 1000);
