 ...
 * @file    Base.c
 *
 * @date     Created on: 2019/05/28
 * @author      haya
 * @note     2019.Dec.26 ビルドワーニング潰し完了
 * @note     2020.01.30  v6 0130ソース反映済み
 * @todo    初期設定値がコード内に即値で埋め込まれているのでヘッダファイル等にdefine定義でまとめる
 */
...
void init_factory_default(uint8_t type, uint8_t mode)
{
    ...
    //SSP_PARAMETER_NOT_USED(mode);       //未使用引数
    uint16_t word_data;
    char str[32];
...
{
    static const UCHAR header[18] = {0x10,0x00,0x56,0x47,0x52,0x50,0x01,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};   
