 ...
 void rfm_program_FW(void)
{
    ...
    if(FLASH_RESULT_BLANK == CheckBlank_data_flash(DATA_FLASH_ADR_RADIO_WRITE_MODE,4))//ブランクでここに来ることは無い
...
Retry_SumVerify:
            ...
            case 7:
                ...
                if(SSP_SUCCESS == rfm_cmd_EndFlashMode()){
                    Printf(" Success\r\n");
                    flash_hp_open();
                    eraseBlock_data_flash(FLASH_UPDATE_EXEC,1);//データフラッシュ64Byteクリア RFM FW更新実行フラグ、RFM書き込み中フラグ クリア
                    flash_hp_close();
                    RfmSequence++;
