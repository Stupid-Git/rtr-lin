/**
 * @file    cmd_thread_entry.c
 *
 * @brief   Cmd Thread entry function
 * @note    2020.01.30  v6 0128ソース反映
 * @note    2020.01.30  v6 0130ソース反映
 * @note    2020.02.07  共有体でのword.lwordアクセスをポインタ（キャスト）アクセスに変更
...
#include "dataflash.h"  //2019.Dec.15 t.saito
#include "Rfunc.h"
#include "Monitor.h"
#include "Suction.h"
#include "Base.h"
...
extern TX_THREAD wifi_thread;       // sakaguchi 2020.09.14


static uint32_t Cmd_Route;          //コマンドの発給元  USB or BLE or TCP or HTTP
static char CmdTemp[300];           //Globals.hから移動
static uint16_t block_no = 0xFFFF;  // 書込成功ブロックNO     // sakaguchi 2021.08.02
...
//未使用static uint32_t CFDebug(void);
...
/**
 * @brief   CMD Thread entry function
 *          このスレッドは、usb_thread,tcp_thread,ble_thread から resumeされる
 *
 * @note   2020.Jun.09 メッセージキューにコマンド処理するバッファへのポインタをセットするようにした
 *                      キューサイズ           16Byte (1メッセージ分)
 *                      メッセージサイズ    4ワード(4Byte×4)
 *
 * @note    md_thread スタックサイズ 8192Byte
 * @attention   printf,sprintf等をサブ関数を含めて大量使用しているのでスタックサイズは大きくする必要がある
 *
 * @note   2020.Jul.15 イベントフラグを待つようにした
 * @note   2020.Jul.15 イベントフラグがからスレッド起床するように変更（イベントフラグ自体は以前からセットしていた）
 * @note   2020.Jul.16 CmdStatusSizeはローカル処理用として各スレッドに返すのはポインタ渡し（メッセージキュー）に変更
 * @note   2020.Jul.16 キューメッセージの送受を専用の構造体に変更
 * @note   2020.Jul.16 メッセージキューで受け取ったコマンド応答用バッファポインタにコマンド応答データをコピーするように変更
 *
 */
void cmd_thread_entry(void)
{
//    uint32_t status = 0;
...
//    static uint32_t cmd_msg[4];        //メッセージキュー受信用
    ...
    for(;;){
        ...
        if(actual_events & FLG_CMD_EXEC){
            tx_queue_receive(&cmd_queue, &cmd_msg, TX_NO_WAIT);
            //Printf("CMD %d\r\n", cmd_msg[0]);
            SET_ERR(CMD, NOERROR);
...
//Printf("%02X %02X %d\r\n", pComRx->command, pComRx->subcommand, pComRx->length);
            ...
            if(-1 == Sum_Crc){
                ...
                DebugLog(LOG_DBG,"[%.5s] %04hd-%04hd (%d)", pComRx->data, (UINT)(ERRORCODE.ERR>>16), (UINT)ERRORCODE.ERR, cmd_msg.CmdRoute);
            }
            else{
            ...
            {
                //DebugLog(LOG_DBG,"HTTP:[%.5s] %04hd-%04hd", pComRx->data, (UINT)(ERRORCODE.ERR>>16), (UINT)ERRORCODE.ERR);
// sakaguchi 2021.08.02 EFIRMはログを圧迫するため出力しない ↓
                if( memcmp(pComRx->data, "EFIRM", 5) != 0 ){
                    PutLog(LOG_SYS,"HTTP:[%.5s] %04hd-%04hd", pComRx->data, (UINT)(ERRORCODE.ERR>>16), (UINT)ERRORCODE.ERR);
                }
// sakaguchi 2021.08.02 ↑
            ...
            //共通処理
            StatusPrintf("STATUS","%04hd-%04hd", (UINT)(ERRORCODE.ERR>>16), (UINT)ERRORCODE.ERR);
 ...
 * @return
 */
static uint32_t CFInitilize(void)
{
	char	*mode;
	int		size;
	uint32_t	Err;
...
// 2023.05.26 ↑
            ...
            tx_event_flags_set (&event_flags_reboot, FLG_REBOOT_REQUEST, TX_OR);        //リブートリクエストイベントシグナル セット
			Err = ERR( CMD, NOERROR );
		}	
            ...
            if(my_config.ble.active != 0){
				...
				tx_event_flags_set( &g_ble_event_flags, FLG_PSOC_RESET_REQUEST, TX_OR); //PSoCリセット要求    2020.Jun.30追加
 ...
 * @return
 */
static uint32_t CFWriteInfo(void)
{
    int Value;
    uint16_t max;
    int Size;
 ...
 * @return
 */
static uint32_t CFReadSettingsFile(void)
{
 ...
 */
static uint32_t CFWriteSettingsFile(void)
{
    char *data;
 ...
 * @attention
 */
static uint32_t CFWriteUnitSettingsFile(void)
{
 ...
 * @note	LO_HIマクロ廃止
 */
static uint32_t CFWriteCertFile(void)
{
 ...
 * @return
 */
static uint32_t CFReadCertFile(void)
{
 ...
 * @return
 */
static uint32_t CFWriteUserDefine(void)
{
 ...
 * @note    LO_HIマクロ廃止
 */
static uint32_t CFReadUserDefine(void)
{
 ...
 * @return
 */
static uint32_t CFBSStatus(void)
{
    //uint16_t val = 1;
    uint16_t phy;
	int swo;
 ...
 * @return
 */
static uint32_t CFNetworkStatus(void)
{
    uint32_t   val;
 ...
 * @note	LO_HIマクロ廃止
 */
static uint32_t CFWriteNetwork(void)
{
    int Size, Value, max;
    char *srv, *id, *pw, *tmp;
 ...
 * @return
 */
static uint32_t CFReadNetwork(void)
{
 ...
 * @note    AP_Count使用
 */
static uint32_t CFReadWalnAp(void)
{
    char ap[16];
    char temp[8];
    int i;
 ...
 * @return
 */
static uint32_t CFWriteDtime(void)
{
 ...
 * @return
 */
static uint32_t CFReadDtime(void)
{
 ...
 * @note	LO_HIマクロ廃止
 */
static uint32_t CFWriteWarning(void)
{
    int Size, Value, max;
    char *tmp;
    uint16_t word_data_a, word_data_b;
 ...
 * @return
 */
static uint32_t CFReadWarning(void)
{
    Printf("    [RWARP exec] \r\n");
 ...
 * @note    AnalyzeCmd() に戻る
 */
static uint32_t CFExtentionSerial(void)
{
    uint32_t Err;
 ...
 * @note    AnalyzeCmd() に戻る
 */
static uint32_t CFExtSettings(void)
{
 ...
 * @note  AnalyzeCmd() に戻る
 */
static uint32_t CFExtSuction(void)
{
    uint32_t    Err;
    int32_t     range,mode;
    uint8_t     sw, para;
 ...
 * @note    AnalyzeCmd() に戻る
 */
static uint32_t CFExtWirelessOFF(void)
{
    uint32_t Err = ERR(CMD, FORMAT);
    uint8_t mode;
 ...
 * @note    AnalyzeCmd() に戻る
 */
static uint32_t CFExtRecStop(void)
{
    uint32_t Err = ERR(CMD, FORMAT);
 ...
 * @note    AnalyzeCmd() に戻る
 */
static uint32_t CFExtAdjust(void)
{
    uint32_t   Err;
    char   *data;
    int     sz_data, act;
 ...
 * @note    AnalyzeCmd() に戻る
 */
static uint32_t CFExtScale(void)
{
    uint32_t   Err;
    char   *data;
    int    sz_data, act;
...
//            if(in_rf_process == false){
            ...
            else if(tx_mutex_get(&g_rf_mutex, WAIT_100MSEC) == TX_SUCCESS)    // ＲＦタスク終了
            {
 ...
 * @note    AnalyzeCmd() に戻る
 */
static uint32_t CFExRealscan(void)
{
    uint16_t i;
 ...
 * @note    AnalyzeCmd() に戻る
 */
static uint32_t CFWirelessRecStop(void)
{
    uint32_t Err = ERR(RF, NOERROR);
 ...
 * @attention case 0の パラメータ違いだけのコピペ関数が13個ある
 */
static uint32_t CFExInterval(void)
{
    uint32_t Err = ERR(RF, NOERROR);
 ...
 * @attention case 0の パラメータ違いだけのコピペ関数が13個ある
 */
static uint32_t CFWirelessRegistration(void)
{
    uint32_t Err = ERR(RF, NOERROR);
...
//            if(in_rf_process == false){
            ...
            else if(tx_mutex_get(&g_rf_mutex, WAIT_100MSEC) == TX_SUCCESS)
			{
                tx_mutex_put(&g_rf_mutex);
 ...
 * @todo    戻り値が違う
 */
static uint32_t CFTestLAN(void)
{
 ...
 * @return  結果（常にNOERROR）
 */
static uint32_t CFTestWarning(void)
{
 ...
 * @note    2021.01.21  追加
 */
static uint32_t  CFEventDownload(void)  
{
    uint32_t Err = ERR(CMD, NOERROR);
    ULONG Flag = FLG_EVENT_SUCTION;
 ...
 */
static uint32_t CFR500Special(void)
{
 ...
 */
static uint32_t CFWriteFirmData(void)
{
    ...
    //static  uint16_t    block_no = 0;       // 書込成功ブロックNO
...
//    if(in_rf_process == false){
    ...
    else if(tx_mutex_get(&g_rf_mutex, WAIT_100MSEC) == TX_SUCCESS)    // ＲＦタスク実行中か？
    {
        tx_mutex_put(&g_rf_mutex);
 ...
 */
static uint32_t CFRoleChange(void)      	//
{
	uint32_t Err;
//	ssp_err_t err;
	Err = ERR(CMD, FORMAT);
 ...
 */
static uint32_t IR_header_read(uint8_t Cmd , uint16_t DataSize)
{
 ...
 */
static uint32_t IR_Nomal_command(uint8_t Cmd, uint8_t SubCmd, uint16_t length, char *pTxData)
{
    optc.cm = Cmd;
    optc.scm = SubCmd;
 ...
 * @note    2020.Jul.06 6個所あったキュー送信、イベントフラグ取得をopt_command_exec()に共通化
 */
static uint32_t IR_64byte_write(char c, char *buf)
{
    optc.cm = c;
    optc.buf = buf;
 ...
 * @note    2020.Jul.06 6個所あったキュー送信、イベントフラグ取得をopt_command_exec()に共通化
 */
static uint32_t IR_64byte_read(char c, char *buf)
{
 ...
 * @note    2020.Jul.06 6個所あったキュー送信、イベントフラグ取得をopt_command_exec()に共通化
 */
static uint32_t IR_data_download_t(char *out_poi, uint8_t sw, uint8_t Cmd, uint64_t GetTime)
{
 ...
 */
static uint32_t IR_soh_direct(comSOH_t *pTx, uint32_t length)
{
 ...
 */
static void StatusPrintf( char *Name, const char *fmt, ... )
{
    va_list args;
 ...
 *          CmdStatusSize   返送パラメータのサイズ（ポインタ）
 */
static void StatusPrintf_S(uint16_t max,  char *Name, const char *fmt,  ... )
{
    va_list args;
