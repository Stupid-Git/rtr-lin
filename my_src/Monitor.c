...
///*  Test!!
			//if(Test == 0){
			    status = tx_event_flags_get (&event_flags_http, (uint32_t)0xffffffff,TX_OR_CLEAR, &actual_events,TX_NO_WAIT);
			//}
			...
			tx_event_flags_set (&event_flags_http, FLG_HTTP_READY, TX_OR);		// HTTPスレッド動作許可:ON
			//if(Test == 0){
			    status = tx_event_flags_get (&event_flags_http, (uint32_t)(FLG_HTTP_SUCCESS | FLG_HTTP_ERROR | FLG_HTTP_CANCEL),TX_OR_CLEAR, &actual_events,TX_WAIT_FOREVER);
			//}
//*/
// hitachi ↑
//sakaguchi ↑
