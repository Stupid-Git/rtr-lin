...
extern pthread_t rf_thread;
extern pthread_mutex_t g_rf_mutex;
extern pthread_mutex_t mutex_sfmem;
extern tdx_flags_t g_wmd_event_flags;
...
//      rf_command = cmd;                                       // コマンド番号セット
        ...
        //TODO tx_thread_resume(&rf_thread);       //RFスレッド起床
		...
		switch(ru_registry.rtr501.header){
			...
			case 0xF9:			// RTR576
				if(EWSTW_Format == 0){
					...
					//TODO	kk_delta_set(ru_registry.rtr501.serial, 0x01);     // 変化あり
					}
				}
				break;
			case 0xF8:			// RTR505
				if(EWSTW_Format == 0){
					if((((EWSTW_Data[17] & 0x01) == 0) && ((EWSTW_Data[30] & 0x01) != 0))	// 設定変更フラグ（記録開始）
					 || ((EWSTW_Data[30] & 0x10) != 0)){									// 設定変更フラグ（表示設定）
					    //TODO	kk_delta_set(ru_registry.rtr505.serial, 0x01);     // 変化あり
					}
				}
				break;
			case 0xF7:			// RTR601
