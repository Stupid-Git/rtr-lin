...
// https://github.com/Pro-YY/eventfd_examples/blob/master/src/eventfd_worker/eventfd_worker.c
    ...
    for (;;) {
        ...
        for (i = 0; i < nfds; i++) {
            if (events[i].events & EPOLLIN) {
                log_debug("[consumer-%d] got event (of %d) from fd-%d", c->rank, nfds, events[i].data.fd);
                ...
                log_debug("[consumer-%d] tasks done: %d, value = %ld", c->rank, ++num_done, v);
            }
        }
    }
}
...
static void *producer_routine(void *data) {
    ...
    while (1) {
        ...
        log_debug("[producer-%d] efd = %d", p->rank, efd);
        event.data.fd = efd;
...
int eventfd_worker_main(int argc, char *argv[])
{
    ...
    for (i = 0; i < NUM_PRODUCERS; i++) {
      ...
      //ret = pthread_create(&p_list[i].thread_id, NULL, producer_routine, &p_list[i]);
        ret = pthread_create(&p_list[i].thread_id, NULL, producer_routine_spike, &p_list[i]);
        if (ret != 0) exit_error("pthread_create");
    }
